...
The first order of business is to get a basic data-type for the regex laid down.
...
A fact that I will be using later to test the implementation is that any
...
regex here, simplifying the code I need to write later.

Lets take a look at what some simple regex would look like in this data type:
...
be represented in this framework, which is a good starting point.

## Simple AST Interpretation

The next order of business is to build a reference matcher implementation. As
...
improving runtimes. Here is my implementation:
...
Theres a lot going on here, so lets take a closer look at each of the component
parts.
...
This is the primary match function that returns whether or not the given regex
and string actually produce a match. The bulk of the work is done in the
`matchStream` function, which produces a list of possible remainder strings
after the regex has done the work of parsing. With that input, all we need to do
is scan the resulting strings and return `True` if we find an empty one,
indicating a full match. `hs|>isJust $ find (== "")` does this, searching
through this list to find the first instance of an empty string, indicating that
the regex has parsed and accepted the entire input.

This is also an instance of using Haskell's lazy evaluation to our advantage. In
a standard strict language, the `matchStream` would have to eagerly calculate
...
size of the `matchStream` result, except in degenerate cases.
...
- `Lit` requires that the string be non-empty, and if it is it advances the
  input one character, failing and terminating the parse in all other cases.
...
produces produce a new list of 1 to `i`. By binding `rep` together multiple
...
By comparison, the `Or` operator is relatively simple. We just combine the lists
produced by the first regex with those of the second, with the same list of
strings as the input. If you're not familiar, `<>` is just list concatenation,
and is a more generic version of `++`, E.G.:
...
[infix](https://wiki.haskell.org/Infix_operator) functions and
...
Having to think up test cases for everything and test them one at a time, manual
testing gets old pretty quickly, at least for me. Luckily, as previously
mentioned, any data that can be represented by the `Regex` type is a valid
regex, perhaps we can use that to automatically test our implementation?
...
specifying test cases and their results by hand.
...
The first thing to note is the `size` parameter and `sized` function. This
...
This size parameter is combined with the `frequency` function to bias the
generation of our data. To explain further, lets start with the simpler `oneof`
function. For example, we can build a simple boolean generator as follows:
...
This setup builds a very simple generator. Each time `generate` is used to
produce a value it chooses, with equal probability, an item in the list and
returns it. This leads into the `frequency` function, that allows us to bias
the generator to produce certain values more often. For example, if we want to
produce `True` with a 75% chance and `False` with a 25% chance we could define
it as follows:
...
resulting structure will be through the `size` and `depth` parameters. `size` is
set by QuickCheck, with a default max of 30. `depth` is our own parameter,
starting at 1 and doubling each time we recurse further into the regex we are
generating. We tie the `depth` parameter to terminating values - `Empty` and
`Lit`, so that as our generator recurses further there is a greater and greater
chance that generation will stop. We pass `size` into the rest of the
...
that the generated regex will recurse deeper to the point where `depth`
overcomes it.
...
λ= generate (resize 10 $ arbitrary :: Gen Regex)
Plus (Star (Mark (Mark (Star (Lit 'e')))))
```

As you can see from these (cherry-picked) examples, small size values tend to
build small structures, whereas larger size values tend to build larger values.
Exactly what we need to build a good `arbitrary`!
...
provided by QuickCheck, however this poses a couple of problems:
...
- The arbitrary instance for `String` will generate arbitrary strings, the vast
  ...
  increasing the length of our testing process.
...
With the ability go generate arbitrary regex and strings from those regex, we
can now start testing. 
...
to generate a string value for the `match` functions second parameter, while
still tying it to the regex that we are testing.
...
This time we see that the test has failed and given us a counterexample regex
...
finds a counterexample, it then runs the `shrink` function to try out different
variations of smaller inputs so that it can give you better results.
...
From there, our base cases are `Lit` and `Empty`, where we return empty lists,
since these are terminating values that can't be shrunk any further.

I also defined the `shrinkOne` function that lets us shrink a regex with a
single argument constructor. In order, it returns:
...
As you can see, when given a regex, the shrink function will try out all
...
This is certainly and improvement, but is still more complex than we'd hope - we
should only see a single `Mark` in our error output.

Well taking a look at the verbose output gives us a hint to the issue:
...
however for the `Mark (Lit 'S')` case the only generated input was `"S"`. To
improve this and increase the odds of getting a minimal counterexample, we can
simply increase the number of strings generated to test our regex:
...
goal is just to define a working regex then this is all you need!
...
by doing [Thompson's construction](https://en.wikipedia.org/wiki/Thompson%27s_construction)
